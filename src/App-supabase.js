...
const HelpMePrayApp = ({ user, setUser }) => {
  ...
  const downloadPrayerImage = async () => {
    ...
    // If we already have a generated preview image, use that instead
    if (generatedImageUrl && generatedImageUrl.startsWith('data:image')) {
      console.log('Using existing generated image for download');
      const link = document.createElement('a');
      link.download = `prayer-${selectedCategory}-${Date.now()}.png`;
  ...
  const downloadImage = async () => {
    ...
    try {
      // Use existing generated image or generate new one
    ...
    return (
      <div style={{
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          {/* App Title with Logo - Perfectly Centered */}
          <div style={{ 
            textAlign: 'center', 
            marginBottom: '30px',
            display: 'flex',
